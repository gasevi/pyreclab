...
PyMethodDef ItemAvg_methods[] =
{
   ...
   { "MAP",       (PyCFunction)ItemAvg_MAP,       METH_VARARGS|METH_KEYWORDS, "calculate Normalized Discounted Cumulative Gain for a user" },
   { "nDCG",      (PyCFunction)ItemAvg_nDCG,      METH_VARARGS|METH_KEYWORDS, "calculate Mean Average Precision for a user" },
   { NULL, NULL, 0, NULL }
};
...
PyObject* ItemAvg_new( PyTypeObject* type, PyObject* args, PyObject* kwdict )
{
   ...
   PyItemAvg* self = reinterpret_cast<PyItemAvg*>( type->tp_alloc( type, 0 ) );
   if( self != NULL )
   {
      ...
      self->m_recAlgorithm = new AlgItemAvg( *self->m_trainingReader, usercol, itemcol, ratingcol );
